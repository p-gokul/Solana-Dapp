  ...
  return (
    <div>
      ...
      <div>
        ...
        />
        <button onClick={findDelegate} disabled={loading || !delegateAddress}>
          {loading ? "Searching..." : "Find Delegated Amount"}
        </button>
      </div>

      {hasSearched && error && <p style={{ color: "red" }}>Error: {error}</p>}
      {hasSearched && !loading && !error && delegationDetails.length === 0 && (
        ...
        <ul>
          {delegationDetails.map((account, index) => {
            ...
            const delegatedAmount =
              ...
              Math.pow(10, parsedInfo.delegatedAmount.decimals);
            ...
            return (
              <li key={index}>
                ...
                <p>
                  <strong>Token Decimals:</strong> {tokenDecimals}
                </p>
                <p>
                  <strong>Delegated Amount:</strong> {delegatedAmount}
                </p>
                <hr />
              </li>
            );
